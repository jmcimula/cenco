...
for (i in 1: 2){
  ...
  var_year <- sample(2000:2005, size=1, replace = FALSE)
  var_month <- sample(1:12, size=1, replace = FALSE)
  var_day <- day_control(var_month, var_year)
  
  birth_year <- sample(1950:1995, size=1, replace = FALSE)
  ...
  geo_infos <- sample(c("johannesburg", "pretoria"), size=1, replace = FALSE)  
  geo_info_loss <- sample(c("johannesburg", "pretoria"), size=1, replace = FALSE)
  marital_status <- sample(c("Single", "Married", "Divorced","Widow"), size=1, replace = FALSE)
  sum_insured  <- sample(2500:10000, size=1, replace = FALSE) 
  claim_serv_prov <- sample(c("ServiceA","ServiceB","ServiceC","ServiceD","ServiceE","ServiceF"), size=1, replace = FALSE)
  ...
  df <- data.frame(
    ...
    f_claim_reason = sample(
      c(
        "Seatbelt", "Speedy","Traffic light", "Pedestrian","Traffic laws",
        "Driving license","Refusal to comply","Holding an outdate insurance",
        "Carrying a Teenager in forward seat","Driving with damaged Winkers"
      ), size = 1, replace = FALSE ),
    date_loss = ymd(paste0(var_year, "-", var_month, "-", var_day)),
    gap = gap,
    gender = gender,
    insured_unique_id = paste0("ZA_IN_KDM",i),
